...
def find_variables(var, text):
    sents = Paragraph(tp._preprocess_text(text)).sentences
    i = 0
    sent = sents[i]
    values = []
    try:
        while len(values) == 0 and i < len(sents):
            sent = sents[i]
            values = mp.get_stoichiometric_values(var, sent.text)
            i = i + 1
    except:
        print('Failed to extract ' + var + ' from text:')
        print(tp._preprocess_text(text))
...
data['abbreviations'] = mp.get_abbreviations_dict(data['targets_upd'], data['paragraphs'])
data['targets_upd_2'] = mp.substitute_abbreviations(data['targets_upd'], data['abbreviations'])
